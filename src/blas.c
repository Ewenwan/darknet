...
// https://blog.csdn.net/tanmx219/article/details/82909538
    for(b = 0; b < batch; ++b){ // 多张图片
        for(k = 0; k < c; ++k){ // 多个通道
            for(j = 0; j < h; ++j){ // 高
                for(i = 0; i < w; ++i){ // 宽
                    int in_index  = i + w*(j + h*(k + c*b));
                    int c2 = k % out_c;    // 分成4个后属于哪个,求具体位置，e.g. 125%64 = 61
                    int offset = k / out_c;// 分成4个后属于哪个 e.g. 125/64=1
...
// 摊平层 把多维的输入一维化，常用在从卷积层到全连接层的过渡  [c w h] ----> 1*c*w*h
    ...
    for(b = 0; b < batch; ++b){
        for(c = 0; c < layers; ++c){
            for(i = 0; i < size; ++i){
                ...
                if (forward) swap[i2] = x[i1];// 前传
                else swap[i1] = x[i2];// 反传
            }
        }
    }
...
{
    float scale = 1./(batch * spatial); // 一个批次图片 的 单个通道  尺寸之和
    int i,j,k;
    for(i = 0; i < filters; ++i){// 外层循环次数为filters，也即mean的维度，每次循环将得到一个平均值
        mean[i] = 0;
        for(j = 0; j < batch; ++j){ // 中层循环次数为batch，也即叠加每张输入图片对应的某一通道上的输出
            for(k = 0; k < spatial; ++k){// 内层循环即叠加一张输出特征图的所有像素值
                int index = j*filters*spatial + i*spatial + k;// 如果理解了上面的注释，下面的偏移是很显然的
                mean[i] += x[index]; // 求和
            }
        }
        mean[i] *= scale;// 求平均   除以该均值所涉及元素的总个数，得到平均值
    }
}
...
void variance_cpu(float *x, float *mean, int batch, int filters, int spatial, float *variance)
{
    ...
    int i,j,k;
    for(i = 0; i < filters; ++i){ // mean维度
        variance[i] = 0;
        for(j = 0; j < batch; ++j){// 叠加每张输入图片对应的某一通道上的输出
            for(k = 0; k < spatial; ++k){// 叠加一张输出特征图的所有像素值
                int index = j*filters*spatial + i*spatial + k;
                variance[i] += pow((x[index] - mean[i]), 2); // 方差：单个元素与均值差 平方和
            }
        }
        variance[i] *= scale;
    }
    ...
    for(b = 0; b < batch; ++b){
        for(f = 0; f < filters; ++f){
            for(i = 0; i < spatial; ++i){
                ...
                //  减去均值除以标准差 来进行归一化
                x[index] = (x[index] - mean[f])/(sqrt(variance[f]) + .000001f); // 分母+一个很小的数，防止除0错误
            }
        }
    }
}
...
float dot_cpu(int N, float *X, int INCX, float *Y, int INCY)
{
    int i;
    float dot = 0;
    for(i = 0; i < N; ++i) dot += X[i*INCX] * Y[i*INCY]; // 点乘
    return dot;
}
...
void softmax(float *input, int n, float temp, float *output)
{
    int i;
    float sum = 0;
    float largest = -FLT_MAX;// 赋初始最大值为float中的最小值-FLT_MAX（定义在float.h中
    // 获取最大值
    for(i = 0; i < n; ++i){
        if(input[i] > largest) largest = input[i];
    }
    for(i = 0; i < n; ++i){
        float e = exp(input[i]/temp - largest/temp);//将每个输入元素减去了该组输入元素中的最大值，以增加数值稳定性
        // 详细请戳：http://freemind.pluskid.org/machine-learning/softmax-vs-softmax-loss-numerical-stability/
        sum += e;// 求和
        output[i] = e;// 并将每一个输入的结果保存在相应的输出中
    }
